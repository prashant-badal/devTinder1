...
.post("/login",async(req,res)=>{
  try{
    ...
    const body1=req.body
    const{password}=body1
      const alreadexistUser = await User.findOne({ email: body1.email });
   ...
   const isPassword = await  alreadexistUser.validPassword(password)
   ...
   if(!isPassword){
      ...
      const token=await jwt.sign ({ _id: alreadexistUser._id }, "BAdal123" )
